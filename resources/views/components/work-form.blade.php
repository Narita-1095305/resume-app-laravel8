...
<div x-data="workHandler()">
    <template x-for="(field, index) in fields" :key="index">
        <div class="border-2 border-black-500 bg-gray-50 p-2 mb-1">
            ...
            <div class="flex mb-2">
                <x-input x-model="field.work_year" class="block w-1/3" min="1960" max="2050" type="number" name="work_year[]"/>
                <p class="block font-medium self-center mx-1 mt-1">年</p>
                <x-input x-model="field.work_month" class="block w-1/4" min="1" max="12"  type="number" name="work_month[]"/> 
                <p class="block font-medium  self-center mx-1 mt-1">月</p>
            </div>
            <input x-model="field.work_background" type="text" name="work_background[]" class="shadow-sm border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500 dark:shadow-sm-light">
